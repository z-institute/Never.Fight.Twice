...
export class Dapp extends React.Component {
  ...
  render() {
    ...
    if (
      ...
      !this.state.tokenIds ||
      !this.state.NFTs ||
      !this.state.NFTs_NeverFightTwice
    ) {
      return <Loading />;
    }
      ...
      <div className="container p-4">
        <div className="row">
          <div className="col-12">
            ...
            <p>
              {" "}
              Welcome! This is an NFT battlefield that can double your NFT or
              lose all! Send an NFT to the pool, and have a chance to win
              another NFT free or lose your NFT! Challenge accepted?
            </p>
            {/* <p>
            ...
            <span>
              You have <b>{this.state.NFTs.length.toString()}</b>{" "}
              {this.state.NFTs.length === 20 ? "or more " : ""} NFTs. The{" "}
              <a
                href={
                  "https://rinkeby.etherscan.io/address/" +
                  this.neverFightTwice.address
                }
              >
                NeverFightTwice contract
              </a>{" "}
              has a total of{" "}
              <b>{this.state.NFTs_NeverFightTwice.length.toString()}</b> NFTs.
              All previously lost NFTs are locked here. Bet now to win them all!
              <span style={{ color: "gray", fontSize: 11.5 }}>
                {" * "}NFTs here are fetched from Opensea, so it may be a bit
                slow to update the latest balances.
              </span>
              <br />
              {this.state.latestBetTx && (
                <div>
                  Your Latest Bet Transaction:{" "}
                  <a
                    href={
        ...
        <div className="row">
          <div className="col-12">
            ...
            */}
            {
              <Mint
                mint={(tokenId) => this._mint(tokenId)}
  ...
  parseLogs(contract, eventName, logs) {
    ...
    return logs
      .filter(
        (log) => log.topics[0] === topic && contract.address === log.address
      )
  ...
  async _intializeEthers() {
    ...
    this.neverFightTwice = new ethers.Contract(
      NeverFightTwiceAddr,
      NeverFightTwiceArt.abi,
      this._provider.getSigner(0)
    );
    this.nftSimple = new ethers.Contract(
      NFTSimpleAddr,
      NFTSimpleArt.abi,
      this._provider.getSigner(0)
    );
  ...
  handleChangeRequestIdUsed(index) {
    this.setState((prev) => ({
      requestIdUsed: prev.requestIdUsed.map((val, i) =>
        !val && i === index ? true : val
      ),
    }));
  }
  ...
  async _updateNFTs() {
    ...
    let response = await fetch(
      `https://testnets-api.opensea.io/api/v1/assets?owner=${this.state.selectedAddress}&order_direction=desc&offset=0&limit=20`,
      options
    );
  ...
  async _updateNeverFightTwiceNFTs() {
    ////////////
    let response = await fetch(
      `https://testnets-api.opensea.io/api/v1/assets?owner=${this.neverFightTwice.address}&order_direction=desc&offset=0&limit=20`,
      options
    );
    let commits = await response.json();
    let len = commits.assets ? commits.assets.length : 0;
  ...
  async _updateBalance() {
    ...
    this._updateNeverFightTwiceNFTs().then((NFTs_NeverFightTwice) =>
  ...
  async _transferTokens(_nftContractAddr, _tokenId, _seed) {
    try {
      ...
      let tx = await nftContract[
        ...
        this.neverFightTwice.address.toString(),
        _tokenId,
        [...Buffer.from(_seed)]
      );
  ...
  async _mint(_number) {
    try {
      ...
      let tx = await this.nftSimple.batchMint(
